...
class Button:
    ...
    def press(self):
        ...
        self.pressed += 1
        if self.caps_lock.pressed % 2 == 0: self.output(self.letter)
        else: self.output(self.letter.upper())
        return self
...
class Keyboard:
    ...
    def __init__(self):
        self.typed = []
        self.keys = {x: Button(x, self.typed.append) for x in LOWERCASE_LETTERS}  # Try a dictionary comprehension!

    def type(self, word):
        """Press the button for each letter in word."""
        assert all([w in LOWERCASE_LETTERS for w in word]), 'word must be all lowercase'
        for x in word: self.keys[x].press()
...
class WinkingBear(Bear):
    ...
    def next_eye(self):
        self.close_eye = not self.close_eye
        return Eye(not self.close_eye)
