...
def eval_and(expressions):
    ...
    while expressions is not nil:
        cur = calc_eval(expressions.first)
        if cur is scheme_f: return scheme_f
        res, expressions = cur, expressions.rest
    return res
...
def eval_define(expressions):
    ...
    2
    """
    symbol, val = expressions.first, expressions.rest
    if val is nil:
        bindings[symbol] = val
    elif val.rest is nil:
        bindings[symbol] = calc_eval(val.first)
    else:
        bindings[symbol] = calc_eval(val)
    return symbol
