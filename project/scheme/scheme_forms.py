...
def do_and_form(expressions, env):
    ...
    # if is_scheme_false(res): return False
    # elif expressions.rest is nil: return res
    # else: return scheme_eval(Pair("and", expressions.rest), env, True)

    res = expressions
    if res is nil: return True
...
def do_or_form(expressions, env):
    ...
    """
    # BEGIN PROBLEM 12
    # if expressions is nil: return False
    # res = scheme_eval(expressions.first, env)
    # if is_scheme_false(res):
    #     return scheme_eval(Pair("or", expressions.rest), env, True)
    # else:
    #     return res
    ...
    while expressions is not nil:
        res = scheme_eval(expressions.first, env, True if expressions.rest is nil else False)
        if not is_scheme_false(res): return res
        expressions = expressions.rest
    return False
    # END PROBLEM 12
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if is_scheme_true(test):
            # BEGIN PROBLEM 13
            if clause.rest is nil: return test
...
def make_let_frame(bindings, env):
    ...
    while cur is not nil:
        ...
        name, val = cur.first.first, scheme_eval(cur.first.rest.first, env)
