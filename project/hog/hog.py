...
def boar_brawl(player_score, opponent_score):
    ...
    "*** YOUR CODE HERE ***"
    ps = player_score % 10
    os = opponent_score // 10 % 10
    return max(3 * abs(ps - os), 1)
    # END PROBLEM 2
...
def sus_points(score):
    ...
    if (num != 3 and num != 4): return score
    while (is_prime(score + 1) != True): score += 1
    return score + 1
    # END PROBLEM 4
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    for i in range(1, 11):
        current = avg(i, dice)
        if (current > num): num, res = current, i
    return res
    # END PROBLEM 9
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    # BEGIN PROBLEM 12
    for i in range(0, 3):
        if (sus_update(0, score, opponent_score) >= 100): return i
    if (sus_update(0, score, opponent_score) >= sus_update(6, score, opponent_score)): return 0
