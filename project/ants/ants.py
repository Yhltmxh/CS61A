...
class Place:
    ...
    def __init__(self, name, exit=None):
        ...
        # BEGIN Problem 2
        if self.exit is not None: self.exit.entrance = self
        # END Problem 2
...
class Ant(Insect):
    ...
    def double(self):
        ...
        # BEGIN Problem 12
        if self.damage is not None and not self.is_double:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        bee_place, cnt = self.place, 0
        while bee_place is not None:
            if cnt > self.upper_bound: return None
            if cnt >= self.lower_bound and bee_place.bees: break
            bee_place, cnt = bee_place.entrance, cnt + 1
        if bee_place is None or bee_place.is_hive: return None
        return random_bee(bee_place.bees) # REPLACE THIS LINE
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        """
        # BEGIN Problem 5
        bees, extra_cnt = list(self.place.bees), 0
        if self.health - amount <= 0: extra_cnt = self.damage
        for bee in bees:
            bee.reduce_health(amount + extra_cnt)
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.turns_to_chew > 0: self.turns_to_chew -= 1
        else:
...
class ContainerAnt(Ant):
    ...
    def store_ant(self, ant):
        # BEGIN Problem 8a
        if self.ant_contained is None: self.ant_contained = ant
        # END Problem 8a
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        while ant_place is not None:
            cur_ant = ant_place.ant
            ant_place = ant_place.exit
            if cur_ant is None: continue
            if cur_ant.is_container and cur_ant.ant_contained is not None:
                cur_ant.ant_contained.double()
            cur_ant.double()
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        # BEGIN Problem Optional 2
        cur_place, dist = self.place, 0
        res = {}
        while cur_place is not None and not cur_place.is_hive:
            ant, bees = cur_place.ant, cur_place.bees
            if ant is not None:
                if dist == 0 and ant.is_container: res[ant] = dist
                elif dist > 0:
                    res[ant] = dist
                    if ant.is_container and ant.ant_contained is not None:
                        res[ant.ant_contained] = dist
            for bee in bees: res[bee] = dist
            cur_place, dist = cur_place.entrance, dist + 1
        return res
        # END Problem Optional 2

    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
        weakness = self.insects_shot * 0.0625 + distance * 0.25
        return max(0, self.damage - weakness)
        # END Problem Optional 2
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        # BEGIN Problem Optional 1
        cur_ant = self.place.ant
        return cur_ant is not None and cur_ant.blocks_path
        # END Problem Optional 1
