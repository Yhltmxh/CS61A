...
def deep_map_mut(func, lnk):
    ...
    while lnk is not Link.empty:
        if isinstance(lnk.first, Link): deep_map_mut(func, lnk.first)
        else: lnk.first = func(lnk.first)
        lnk = lnk.rest
...
def two_list(vals, counts):
    ...
    """
    v_len, res = len(vals), Link.empty
    for i in range(v_len):
        for _ in range(counts[v_len - i - 1]):
            res = Link(vals[v_len - i - 1], res)
    return res
